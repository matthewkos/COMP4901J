...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        layer1_flatten = np.reshape(layer1, (-1, self.params['W2'].shape[0]))
        layer2, l2_cache = affine_relu_forward(layer1_flatten, self.params['W2'], self.params['b2'])
        layer3, l3_cache = affine_forward(layer2, self.params['W3'], self.params['b3'])
        scores = layer3
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dlayer3, dw3, db3 = affine_backward(dout, l3_cache)
        dlayer2, dw2, db2 = affine_relu_backward(dlayer3, l2_cache)
        dlayer2_reshape = np.reshape(dlayer2, layer1.shape)
        dlayer1, dw1, db1 = conv_relu_pool_backward(dlayer2_reshape, l1_cache)

        grads['W3'] = dw3 + self.reg * l3_cache[1]
        grads['b3'] = db3
        grads['W2'] = dw2 + self.reg * l2_cache[0][1]
        grads['b2'] = db2
