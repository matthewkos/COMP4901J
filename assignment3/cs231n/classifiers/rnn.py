...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        embedded_layer, cache_emb = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == 'rnn':
            rnn_layer, cache_rnn = rnn_forward(embedded_layer, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            rnn_layer, cache_rnn = lstm_forward(embedded_layer, h0, Wx, Wh, b)
        else:
            raise ValueError('Invalid cell_type "%s"' % self.cell_type)
        scores, cache_scores = temporal_affine_forward(rnn_layer, W_vocab, b_vocab)
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'lstm':
            next_c = np.zeros((N, Wh.shape[0]))
        for t in range(max_length):
            c0 = W_embed[prev_word, :].reshape(N, -1)
            if self.cell_type == 'rnn':
                next_h, _ = rnn_step_forward(c0, next_h.reshape(N, -1), Wx, Wh, b)
            elif self.cell_type == 'lstm':
                next_h, next_c, _ = lstm_step_forward(c0, next_h.reshape(N, -1), next_c, Wx, Wh, b)
